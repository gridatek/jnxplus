...
describe('nx-maven spring-boot-4 parent-pom e2e', () => {
  ...
  beforeAll(async () => {
    ...
    await runNxCommandAsync(
      `generate @jnxplus/nx-maven:init --dependencyManagement spring-boot-4-parent-pom`,
    );
  }, 120000);
  ...
  it('should init the workspace with spring-boot-4-parent-pom', async () => {
    ...
    // Check pom.xml contains Spring Boot 4 parent
    const pomXml = readFile('pom.xml');
    expect(pomXml.includes('spring-boot-starter-parent')).toBeTruthy();
    expect(pomXml.includes('4.0.0')).toBeTruthy();
  }, 120000);

  it('should create a java application with spring-boot-4', async () => {
    const appName = uniq('m-sb4-app-');

    await runNxCommandAsync(
      `generate @jnxplus/nx-maven:application ${appName} --framework spring-boot-4`,
    );
  ...
  it('should create a kotlin application with spring-boot-4', async () => {
    ...
    await runNxCommandAsync(
      `generate @jnxplus/nx-maven:application ${appName} --framework spring-boot-4 --language kotlin --port ${port}`,
    );
  ...
  it('should create a java library with spring-boot-4', async () => {
    const libName = uniq('m-sb4-lib-');

    await runNxCommandAsync(
      `generate @jnxplus/nx-maven:library ${libName} --framework spring-boot-4`,
    );
  ...
  it('should create a kotlin library with spring-boot-4', async () => {
    ...
    await runNxCommandAsync(
      `generate @jnxplus/nx-maven:library ${libName} --framework spring-boot-4 --language kotlin`,
    );
  ...
  it('should add a lib to an app dependencies', async () => {
    ...
    await runNxCommandAsync(
      `generate @jnxplus/nx-maven:library ${libName} --framework spring-boot-4 --projects ${appName}`,
    );
